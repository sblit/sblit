...
public class FileSender {
	 ...
	 * @param password
	 * Used for encryption of the own files to send. If you just want to send foreign files this can be null.
	 * @param app
	 ...
	 */
	public void sendOwnFiles(String[] receivers, File[] filesToPush, File logFile) throws IOException, BufException{
		for(File f : filesToPush){
			byte[] ownAddress = this.ownAddress.getBytes();
			byte[] logFileContent = Files.readAllBytes(Paths.get(logFile.getAbsolutePath()));
			byte[] fileContent = Files.readAllBytes(Paths.get(f.getAbsolutePath()));
			byte[] filePath = f.getAbsolutePath().replaceAll(dataDirectory.getAbsolutePath(), "").getBytes();
			//TODO hashcode
			byte[] hash = Converter.intToByteArray(filePath.hashCode());
					
			//TODO encryption
			SymmetricEncryption encryption = new SymmetricEncryption(password);
			byte[] encryptedLogFileContent = encryption.encryptByteArray(logFileContent);
			byte[] encryptedFileContent = encryption.encryptByteArray(fileContent);
			byte[] encryptedFilePath = encryption.encryptByteArray(filePath);
			
			byte[] data = new byte[ownAddress.length + hash.length + encryptedLogFileContent.length + encryptedFileContent.length + encryptedFilePath.length + 4];
			...
			data[ownAddress.length + hash.length + 2 + encryptedLogFileContent.length] = (byte)'\0';
			for (int i = 0; i < encryptedFileContent.length; i++)
				data[i + 3 + ownAddress.length + hash.length + encryptedLogFileContent.length] = encryptedFileContent[i];
			data[ownAddress.length + hash.length + encryptedLogFileContent.length + encryptedFileContent.length + 3] = (byte)'\0';
			for(int i = 0; i < encryptedFilePath.length;i++)
				data[i + 4 + ownAddress.length + hash.length + encryptedLogFileContent.length + encryptedFileContent.length] = encryptedFilePath[i];
	 ...
	 */
	public void sendForeignFiles(String[] receivers, File[] filesToPush) throws IOException, BufException{
		for(File f : filesToPush){
	...
	private void send(byte[] data, String[] receivers) throws BufException{
		for(String receiver : receivers){
			...
			//TODO hashcode
			app.send(Converter.intToByteArray(data.hashCode()), Sblit.APPLICATION_IDENTIFIER, receiver );
		}
	}
